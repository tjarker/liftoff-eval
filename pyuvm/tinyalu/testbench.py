...
class BenchmarkSeq(uvm_sequence):
    async def body(self):
        seqr = ConfigDB().get(None, "", "SEQR")
        reps = ConfigDB().get(None, "", "REPS")
        for _ in range(reps):
            uvm_root().logger.info(f"Benchmark iteration {_}")
            random = RandomSeq("random")
            max = MaxSeq("max")
            fib = FibonacciSeq("fib")
            await random.start(seqr)
            await max.start(seqr)
            await fib.start(seqr)
...
@pyuvm.test()
class BenchmarkTest(AluTest):
    """Run benchmark with random and max values"""

    def build_phase(self):
        ConfigDB().set(None, "*", "REPS", 100000)
        uvm_factory().set_type_override_by_type(TestAllSeq, BenchmarkSeq)
        return super().build_phase()